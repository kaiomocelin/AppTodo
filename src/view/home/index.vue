    ...
    <div class="linha">
      <div class="colunaA">
        <ul v-for="(item, key) in todos" :key="key">
          <li v-show="!item.completed" @click="completed = !completed" >
            <span :class="completed ? 'completed': ''">{{ item.title }}
            <b-button class="btnRemove" @click="remove(key)"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16">
